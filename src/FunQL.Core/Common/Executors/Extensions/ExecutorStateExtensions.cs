...
namespace FunQL.Core.Common.Executors.Extensions;

/// <summary>Extensions for <see cref="IExecutorState"/>.</summary>
public static class ExecutorStateExtensions
{
    ...
    public static T? FindContext<T>(this IExecutorState state) where T : IExecuteContext =>
        state.FindContext<IExecuteContext, T>();
    ...
    public static IExecuteContext RequireContext(this IExecutorState state, Type type) =>
        state.RequireContext<IExecuteContext>(type);
    ...
    /// <returns>The found context.</returns>
    /// <exception cref="ArgumentException">If context was not found.</exception>
    public static T RequireContext<T>(this IExecutorState state) where T : IExecuteContext =>
        state.RequireContext<IExecuteContext, T>();
}