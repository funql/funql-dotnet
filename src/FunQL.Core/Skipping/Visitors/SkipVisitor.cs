...
/// <summary>Default implementation of <see cref="ISkipVisitor{TState}"/>.</summary>
/// <param name="constantVisitor">Visitor to delegate <see cref="IConstantVisitor{TState}"/> implementation to.</param>
/// <inheritdoc/>
public class SkipVisitor<TState>(
    IConstantVisitor<TState> constantVisitor
) : ISkipVisitor<TState> where TState : IVisitorState
{
    ...
    /// <inheritdoc/>
    public virtual Task Visit(Skip node, TState state, CancellationToken cancellationToken) =>
